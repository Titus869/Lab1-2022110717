...
public class WordGraph {
    ...
    public static void main(String[] args) throws IOException {
        ...
        // 构建有向带权图，用 LinkedHashMap 保留 from 的插入顺序
        for (int i = 0; i < words.length - 1; i++) {
            String from = words[i], to = words[i + 1];
            if (from.isEmpty() || to.isEmpty()) continue;

            // 如果是第一次遇到 from，就创建一个 LinkedHashMap 作为它的邻接表
            graph.computeIfAbsent(from, k -> new LinkedHashMap<>())
    ...
    /**
     * 功能1：打印全局图的邻接表（带权重）。
     * 因为 graph 是 LinkedHashMap，遍历 entrySet() 自然就是第一次插入的顺序。
     */
    ...
    static String generateNewText(String inputText) {
        String cleaned = inputText.replaceAll("[^a-zA-Z\\s]", " ")
                                  .toLowerCase();
        String[] words = cleaned.split("\\s+");
    ...
    static String calcShortestPath(String word1, String word2) {
        ...
        if (!graph.containsKey(start)) return "No word \"" + word1 + "\" in the graph!";
        if (!graph.containsKey(end)) return "No word \"" + word2 + "\" in the graph!";
